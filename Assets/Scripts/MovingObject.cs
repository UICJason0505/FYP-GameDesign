...
public class MovingObject : MonoBehaviour
{
    ...
    private LineRenderer lineRenderer; // LineRenderer 用来绘制路径
    private int currentAP; // 记录玩家的行动点
    private enum ObjectState { None, Selected, Dragging }; // 物体状态枚举
...
public class MovingObject : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // 模式切换时避免冲突
        if (Input.GetKeyDown(KeyCode.X) || Input.GetKeyDown(KeyCode.C) || Input.GetKeyDown(KeyCode.V)) HandleModeSwitch();
    ...
    // Handle mode switching (X, C, V keys)
